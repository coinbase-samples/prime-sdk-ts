...
const getFcmRiskLimits = async (financingService, entityId) => {
  const request = {
    entityId,
  };

  console.log('Getting FCM risk limits for entity: ', entityId);
  const response = await financingService.getFcmRiskLimits(request);
  return response;
};
...
getFcmRiskLimits(financingService, entityId)
  .then((response) => {
